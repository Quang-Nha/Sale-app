...
<h1 class="text-center text-danger">THỐNG KÊ DOANH THU THEO THỜI GIAN(NGÀY)</h1>
<br>
...
<form action="" method="get">
    <div class="form-group">
        ...
        <label class="w-100">Sản phẩm cần thống kê theo thời gian(ngày)
...
<%-- phần vẽ biểu đồ --%>
<div>
    <canvas id="myProductDayStatsChart"></canvas>
</div>
...
<table class="table table-hover">
    ...
    <%-- lặp qua danh sách lấy ra các mảng Object[] nó chứa,
         tại mỗi mảng lấy ngày, tháng, năm cho vào cột 1 và
         tổng doanh thu của ngày tháng, năm đó cho vào cột 2 để hiển thị --%>
    <c:forEach var="pds" items="${productDayStats}">
        <tr>
            <td>${pds[0]}/${pds[1]}/${pds[2]}</td>
                <%-- fomat dạng kiểu số có dấu . đánh dấu phần nghìn và bỏ phần thập phân --%>
            <td><fmt:formatNumber type="number" maxFractionDigits="0" value="${pds[3]}"/>.000.000 VND</td>
        </tr>
    </c:forEach>
</table>

<script>
    // tạo 2 biến mảng chứa danh sách 1 là ngày, tháng, năm và 2 là danh sách doanh thu của ngày, tháng, năm  đó
    let dateLabels = [], amountDateInfo = [];

    // lặp qua model chứa list dữ liệu đã truy vấn
    // thêm các dữ liệu vào 2 mảng tương ứng
    <c:forEach var="pds" items="${productDayStats}">
    dateLabels.push('${pds[0]}/${pds[1]}/${pds[2]}');
    amountDateInfo.push(${pds[3]});
    </c:forEach>
    ...
    window.onload = function () {
        ...
        productDayMonthChart("myProductDayStatsChart", dateLabels, amountDateInfo);
    };
</script>
